 ...
 (function thr:mkpool (endgame)
   ("Makes a thread pool."
    ...
    (let* ((man (thr:mkmanual))
           (mtx (thr:mkmutex))
           (egmtx (thr:mkmutex))
           (thctr (noconst (cons nil 0)))
           (stack (noconst (cons nil nil)))
           (altadd (noconst (cons nil nil)))
           (incthctr (fun ()
...
;; N.B.: when used with a thread pool the controller blocks if the pool is empty
 (function thr:mkqueue (nthr? consumer)
   ("Makes a consumer queue with a given consumer processor function."
    ...
    (let* ((mtx (thr:mkmutex))
           (man (thr:mkmanual))
           (qhead (noconst (cons nil nil)))
           (qtail (noconst (cons nil nil)))
           (kil   (noconst (cons nil nil))) ;; the last words to say
           (add (fun (x)
                   (thr:mutex_wait mtx)
                   (let ((nw (cons x nil)))
                     (if (null? (cdr qhead))
